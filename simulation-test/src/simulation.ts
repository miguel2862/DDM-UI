...
import type {
  NPE, Connection, NPERow, ConnectionRow,
  SimulationOptions, SimulationOutputRow
} from './types.js';
import { RNG } from './random.js';

// ── Helper: Logistic function (R line 85-87) ────────────────────────────────
function L(x: number, sigma: number): number {
  return 1 / (1 + Math.exp((-x + 0.5) / sigma));
}

// ── Helper: Estimate Beta distribution parameters (R line 89-95) ─────────────
function estBetaParams(mu: number, sigma: number): { alpha: number; beta: number } {
  const variance = sigma * sigma;
  const a = mu * (mu - mu * mu - variance) / variance;
  const b = (mu * (1 - mu) / variance - 1) * (1 - mu);
  if (a <= 0 || b <= 0) {
    throw new Error(
      'This combination of mean and standard deviation for the threshold results in invalid parameters for a beta distribution'
    );
  }
...
// ═══════════════════════════════════════════════════════════════════════════════
export function simulateDBP(
  npesData: NPERow[],
  connectionsData: ConnectionRow[],
  timeStepsRaw: Record<string, unknown>[],
  hasITI: boolean[],
  ...
  if (npesData.length === 0) throw new Error('NPEs array is empty');
  if (connectionsData.length === 0) throw new Error('Connections array is empty');

  // ── Initialize network (R lines 140-172) ────────────────────────────────────
  const network: Record<string, NPE> = {};
  const networkOrder: string[] = []; // preserve insertion order
  ...
  const isInput: Record<string, boolean> = {};
  ...
  const saveElements: Set<string> = new Set();

  if (options.saveData?.Elements) {
    for (const el of options.saveData.Elements) saveElements.add(el);
  } else {
  ...
  for (const name of networkOrder) {
    if (isInput[name]) saveElements.add(name);
  }

  // ── Parse TimeSteps table (convert from raw records) ────────────────────────
  interface ParsedTS {
    Phase: string;
    Trial: number;
    TimeStep: number;
  ...
  for (const row of timeStepsRaw) {
    ...
    const stimuli: Array<{ npe: string; activation: number }> = [];
  ...
  const saveTimeSteps = new Set<number>();
  if (options.saveData?.TimeSteps) {
    for (const ts of options.saveData.TimeSteps) saveTimeSteps.add(ts);
  } else {
    for (const row of timeSteps) saveTimeSteps.add(row.TimeStep);
  }

  // ── Allocate output data (R lines 194-195) ──────────────────────────────────
  const dataSim: SimulationOutputRow[] = [];
  const saveElementsList = Array.from(saveElements);

  // ── ComputeInputs (R lines 70-83) ──────────────────────────────────────────
  function computeInputs(npe: NPE): [number, number] {
    let exc = 0;
    let inh = 0;
  ...
  function dCA1(dVTAVal: number, previousdCA1: number): number {
    let d = 0;
    let n = 0;
    for (const name of networkOrder) {
  ...
  function computeR(npeName: string): [number, number] {
    let sumWeightsExc = 0;
    let sumWeightsInh = 0;
  ...
  for (let ts = 0; ts < timeSteps.length; ts++) {
    ...
    let outputRow: SimulationOutputRow | null = null;
    if (shouldSave) {
      outputRow = {
        Phase: row.Phase,
        Trial: row.Trial,
