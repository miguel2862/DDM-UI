...
function makeNPEs(specs, defaults = {}) {
    const { activation = 0, ts = 0.1, decay = 0.1, mu = 0.2, sigma = 0.15, logisSigma = 0.1 } = defaults;
    return specs.map(([NPE, Type, Layer]) => ({
        NPE, Type: Type, Layer: Layer,
        Activation: activation, 'Temporal.Summation': ts, 'Activation.Decay': decay,
        mu, sigma, logisSigma,
    }));
}
// ── Helper to create Connection rows ──────────────────────────────────────────
function makeConns(specs, defaults = {}) {
...
// Templates
// ═══════════════════════════════════════════════════════════════════════════════
export function getExtinctionTemplate() {
    return {
        id: 'extinction',
        name: 'Extinction',
        npes: makeNPEs([
...
export function getAcquisitionTemplate() {
    return {
        id: 'acquisition',
        name: 'Acquisition',
        npes: makeNPEs([
...
export function getSpontaneousRecoveryTemplate() {
    return {
...
export function getLatentInhibitionTemplate() {
    return {
...
export function getBlockingTemplate() {
    const npes = makeNPEs([
        ['US', 'Excitatory', 'US'],
        ['D', 'Excitatory', 'Dopaminergic'],
