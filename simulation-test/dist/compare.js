...
import { getExtinctionTemplate, getAcquisitionTemplate, getBlockingTemplate, getSpontaneousRecoveryTemplate, getLatentInhibitionTemplate, } from './templates.js';
const N_RUNS = 20; // number of networks per template
// ── Helper: compute mean activation per trial for a given NPE ─────────────────
function meanPerTrial(results, phase, npe, maxTrial) {
    const means = [];
...
function runMultiple(template, n) {
    const allResults = [];
    for (let i = 0; i < n; i++) {
