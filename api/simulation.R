# DDM Simulation Engine
# Extracted from DDM_UI (2026).R
  ...
  tryCatch(
    {
      ...
      for (ts in 1:nrow(TimeSteps)) {
        ...
        # Initialize learning signals for this timestep
        if (TimeSteps[ts, "TimeStep"] %in% saveData$TimeSteps) {
          data.sim[t, "dVTA"] <- 0
          data.sim[t, "dH"] <- 0
        }
        ...
        if (LearningRuleIsActive) {
          ...
          # Save computed learning signals
          if (TimeSteps[ts, "TimeStep"] %in% saveData$TimeSteps) {
            data.sim[t, "dVTA"] <- dD
            data.sim[t, "dH"] <- dH
          }

          # PUpdate procedure: order NPEs
          ...
          for (i in scrambledNPEs) {
            ...
            # PUpdate procedure: order connections
            if (pupdate == "async_random" || pupdate == "sync_random") {
