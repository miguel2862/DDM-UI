# Experiment D: Linear vs Exponential decrement
# The current decrement uses: weight - beta * weight * apre * apost (exponential decay)
# Test linear: weight - beta * apre * apost
...
# Create a modified version of Simulate.DBP with LINEAR decrement
...
cat("=== EXPERIMENT D: Linear vs Exponential Decrement ===\n\n")
...
cat("--- Exponential decrement (original) ---\n")
...
for (type in c("Exponential", "Linear")) {
  subset <- results_D[results_D$Type == type, ]
  cat(sprintf("\n%s decrement:\n", type))
  ...
  cat(sprintf("  Mean pct change:              %.1f%% (SD=%.1f%%)\n", mean(subset$Pct_change), sd(subset$Pct_change)))
  cat(sprintf("  Mean %%dVTA > disc:            %.1f%% (SD=%.1f%%)\n", mean(subset$Pct_dVTA_above_disc), sd(subset$Pct_dVTA_above_disc)))
}

# Also report all connection weights at end of extinction for linear version
cat("\n--- All connection weights at end of extinction (Linear, last network) ---\n")
