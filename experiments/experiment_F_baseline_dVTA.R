# Experiment F: Running Average Baseline for dVTA (DTD Adjusted Fix)
#
# Problem: During extinction, D resets to ~0 at trial start, then M''1 drives D up
#          creating a large positive dVTA that triggers INCREMENT instead of DECREMENT.
#
# Fix: Instead of dVTA = D(t) - D(t-1), use:
#      dVTA = D(t) - D_baseline
#      where D_baseline is an exponential moving average of D that captures
#      the "expected" dopamine level. When D_baseline is high (after training),
#      even if D activates, dVTA will be near zero or negative.
#
# This is analogous to the Rescorla-Wagner prediction error:
#   λ - ΣV where λ is actual US and ΣV is predicted US
...
# Modified Simulate.DBP with running-average dVTA baseline
Simulate.DBP.Adjusted <- function(NPEs, Connections, TimeSteps, HasITI = c(FALSE),
                                   disc = 0.001, threshold = "gaussian",
                                   pupdate = "async_random",
                                   saveData = list(),
                                   # NEW PARAMETERS for DTD Adjusted:
                                   dVTA_baseline_rate = 0.05, # how fast baseline tracks D
                                   use_baseline_dVTA = TRUE) {
  ...
  tryCatch({
    ...
    # MODIFIED dVTA: uses running average baseline
    D_baseline <- 0  # running average of D activation

    dVTA_adjusted <- function() {
      d_current <- 0
      n <- 0
      for (i in 1:length(network)) {
        if (network[[i]]@Layer == "Dopaminergic") {
          d_current <- d_current + network[[i]]@Activation
          n <- n + 1
        }
      }
      if (n == 0) return(0)
      d_current <- d_current / n

      if (use_baseline_dVTA) {
        # Prediction error = actual - expected
        prediction_error <- d_current - D_baseline
        # Update baseline: exponential moving average
        D_baseline <<- D_baseline + dVTA_baseline_rate * (d_current - D_baseline)
        ...
        for (i in 1:length(network)) {
          if (network[[i]]@Layer == "Dopaminergic") {
            d_prev <- d_prev + network[[i]]@PreviousActivation
          }
        }
        d_prev <- d_prev / n
    ...
    current.phase <- 1
    t <- 0
    D_baseline <- L(0, network[[1]]@logisSigma)  # Start at baseline activation
          ...
          for (j in scrambledConnections) {
            ...
            if (network[[pre]]@Layer != "US") {
              if (d >= disc) {
                ...
                # SAME decrement as original (exponential)
                network[[i]]@InputConnections[[j]]@weight <- network[[i]]@InputConnections[[j]]@weight -
...
cat("=== EXPERIMENT F: Running-Average Baseline dVTA ===\n")
cat("Fix: dVTA = D(t) - D_baseline instead of D(t) - D(t-1)\n")
cat("D_baseline updated via exponential moving average\n\n")

# Test with several baseline rates
for (rate in c(0.01, 0.02, 0.05, 0.1, 0.2)) {
  cat(sprintf("--- Baseline rate = %.2f ---\n", rate))
  ...
  for (net in 1:n_networks) {
    ...
    result <- Simulate.DBP.Adjusted(
      NPEs = template$npes,
      Connections = template$connections,
      TimeSteps = TimeSteps,
      HasITI = template$hasITI,
      disc = 0.001,
      dVTA_baseline_rate = rate,
      use_baseline_dVTA = TRUE
    )
  ...
  cat(sprintf("  Mean %%dVTA>disc:        %.1f%%\n", mean(results$Pct_dVTA_above_disc)))
...
# Show per-trial trajectory for best rate
cat("\n=== Per-trial trajectory (rate=0.05, net 1) ===\n")
...
result <- Simulate.DBP.Adjusted(
  ...
  disc = 0.001, dVTA_baseline_rate = 0.05, use_baseline_dVTA = TRUE
)
...
# Show last 5 training trials and first 20 + last 10 extinction trials
cat("Phase     Trial  M''1->D\n")
for (trial in c(96:100)) {
  rows <- training_rows[result[training_rows, "Trial"] == trial]
  if (length(rows) > 0) {
    cat(sprintf("Training  %3d    %.4f\n", trial, result[max(rows), m1d_col]))
  }
}
cat("---\n")
for (trial in c(1:20, 50, 60, 70, 80, 90, 100)) {
  rows <- extinction_rows[result[extinction_rows, "Trial"] == trial]
  if (length(rows) > 0) {
    cat(sprintf("Extinct   %3d    %.4f\n", trial, result[max(rows), m1d_col]))
  }
}
